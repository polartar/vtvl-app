...
export default function useToggle(
  initialState = false
): [state: boolean, toggle: () => void, turn: (turnTo: boolean) => void, turnOn: () => void, turnOff: () => void] {
  const [state, change]: [boolean, (turnTo?: boolean) => void] = useReducer(
    (state_: boolean, turnTo?: boolean) => turnTo ?? !state_,
    initialState
  );
