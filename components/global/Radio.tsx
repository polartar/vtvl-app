  ...
  return (
    <label
      className={`flex-row cursor-pointer ${
        variant === "input-style"
          ? "radioButton flex-row cursor-pointer"
          : "gap-3 items-start"
      }`}
    >
      <div
      ...
      >
        {props.checked && variant === "input-style" ? (
          <img src="/icons/check.svg" alt={label} />
        ) : null}
        {props.checked && variant === "default" ? (
          <div className="w-2 h-2 bg-secondary-900 rounded-full"></div>
        ) : null}
      </div>
