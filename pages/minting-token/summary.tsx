    ...
    <div className="panel rounded-lg mx-auto max-w-xl w-1/2 mt-14">
      ...
      <div className="border-y border-gray-300 mt-5 py-5 grid md:grid-cols-3">
        <label>
          ...
          <p className="paragraph-small-medium capitalize">{supplyCap.toLowerCase()}</p>
        </label>
        <label>
          <span>Amount to mint</span>
          <p className="paragraph-small-medium">{formatNumber(+initialSupply)}</p>
        </label>
        <label>
          <span>Maximum amount</span>
          <p className="paragraph-small-medium">{supplyCap === 'LIMITED' ? formatNumber(+maxSupply) : 'Unlimited'}</p>
        </label>
      </div>
